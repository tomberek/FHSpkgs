...
pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    # attr is acl's real dependency, built from source first. Re-snapshot
    # right after, so acl's own installOnlyNew below excludes attr's
    # files too -- an environment composer is expected to pull attr in
    # as its own dependency edge (see attr-fhs.nix), not have it
    # silently duplicated into every consumer's output.
